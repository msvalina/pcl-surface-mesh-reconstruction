...
Program RGBDSlam omogućuava brzo prikupljanje 3D modela objekata i scena
u unutršnjosti prostorija (oblak točaka u boji) upravljanjem rukom
kamerom tipa Kineckt. Razvijen je suradnjom
...
System'' autora Endres, Hess, Burgard, Engelhard, Cremers, Sturm.
}
...
sparuju s značajkama iz prethodnih slika. Drugi dio ispituje dubinu
slika na lokacijama izračunatih značajki. Time dobiva znanje o točkinim
3D korespodencijama između bilo koje dvije sličice. Zatim s tim
korespodencijama estimira relativnu transformaciju između sličica
koristeći RANSAC\footnotemark[3]. Kako parovi estimiranih poza između
sličica nisu globalno konzistentni treći dio programa optimizira graf
poza koristeći g\textsuperscript{2}o\footnotemark[4]. Algoritam u ovoj
fazi daje globalno konzistentni 3D model promatrane okoline predstavljen
oblakom točaka u boji. Takav oblak točaka je korišten dalje u diplomskom
radu za stvaranje mreže trokuta. Zadnji četvrti dio koristi
...
\footnotetext[2]{%
RGBDSlam se oslanja na OpenCV biblioteku u kojoj su implementirani SURF,
SIFT i ORB algoritmi za pronalazak značajki.}
\footnotetext[3]{%
RANSAC Random Sample Consensus je iterativna metoda estimiranja parametara
matematičkog modela iz promatranih mjerenja koji sadržavaju šum.
}
\footnotetext[4]{%
\href{http://openslam.org/g2o.html}%
{g\textsuperscript{2}o:} A General Framework for Graph Optimization je
okvir otvorenog koda za optimiziranje graph-based? nelinearnih error?
funkcija.}
\footnotetext[5]{%
\href{http://octomap.github.io/}%
{OctoMap} je efikasni probabilistički okvir za 3D mapiranje baziran na
octree strukturi.}
...
Prilikom korištenja RGBDSlama nije bilo potrebe za mjenjanjem zadanih
...
podataka. Dok ručni način korisniku omogućava na pritisak tipke Enter
uzimanje slike. Za potrebe diplomskog rada korišten je ručni način rada.
...
Za snimanje scena je korišten ručni način rada RGBDSlam programa.
...
kamerom, gledat u računalo i pritiskat Enter za slikanje. Zato je pri
snimanju scena sudjelovalo više osoba ili je korištena
skripta\footnotemark[1] koja je umjesto korisnika stiskala Enter nakon
proizvoljnog broja sekudni. Tijekom snimanja trebalo je obratit
pozornost na značajke scene koja se snima kako bih program mogao spariti
značajke s prethodnom scenom. Tijekom izrade diplomskog rada snimljeno
je osam scena odnosno prostorija koje su obrađene u
...
Predložak takvog koda se nalazi u ispisu koda~\ref{lstUcitavanjeOblaka}
Nakon učitavanja oblaka točaka \texttt{reader} objektom, nad njim se
vrše operacije npr. reduciranje oblaka točaka. Nakon toga kreiranjem
i korištenjem \texttt{writer} objekta promjenji oblak se zapisuje u datoteku.
...
\begin{lstlisting}[label=lstUklanjanje, caption={Dio izvornog koda o
    uklanjanju šuma iz funkcije \texttt{remove\_outliers()} }]
    // Create the filtering object
...
{\texttt{pointclouds.org/doc/\dots/kdtree\_search.php}}}
\footnotetext[2]{%
...
{\texttt{pointclouds.org/doc/\dots/normal\_estimation.php}}}
